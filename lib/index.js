...
var forecastWeatherUrl = "http://api.openweathermap.org/data/2.5/forecast?units=imperial&cnt=2&";
...
function ION(identifier) {
  ...
  this.setWeather = function(current_temp, current_conditions, future_temp, future_conditions, sunrise_24hr, sunrise_min, sunset_24hr, sunset_min, cb) {
    ...
    // always send time before weather
    this.sendTimePacket(function(err) {
      if (err) {
        cb(err);
      } else {
        var reqId = self.assignCallback(cb);
        self.sendPacket(packets.makeSetWeatherPacket(reqId, current_temp, current_conditions, future_temp, future_conditions, sunrise_24hr, sunrise_min, sunset_24hr, sunset_min));
      }
    });
  }
