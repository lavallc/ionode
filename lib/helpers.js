...
function getMoodForName(moodName) {
	...
	for (mood in ionNotifications) {
		// this allows names with or without spaces, with or without capitals
		if (ionNotifications[mood].name.toLowerCase().replace(/\s+/g, '') === moodName.toLowerCase().replace(/\s+/g, ''))
			return ionNotifications[mood];
	}
...
function getMoodConfigForName(moodName, configName) {
	...
	for (mood in ionNotifications) {
		// this allows names with or without spaces, with or without capitals
		if (ionNotifications[mood].name.toLowerCase().replace(/\s+/g, '') === moodName.toLowerCase().replace(/\s+/g, '')) {
			for (config in ionNotifications[mood].configs) {
				// this allows names with or without spaces, with or without capitals
				if (ionNotifications[mood].configs[config].name.toLowerCase().replace(/\s+/g, '') == configName.toLowerCase().replace(/\s+/g, ''))
					return ionNotifications[mood].configs[config];
			}
		}
	}
